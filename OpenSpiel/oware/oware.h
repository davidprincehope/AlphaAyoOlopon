...
class OwareState : public State {
 public:
  ...
  Player CurrentPlayer() const override {
    // OpenSpiel uses a special player id at terminal states. Otherwise the
    // player stored in the board is the player who chooses the next action.
 ...
 private:
  ...
  int LowerHouse(int house) const {
    // Houses are grouped in rows of num_houses_per_player_ entries. This
    // rounds a physical house index down to the first house in its row.
  ...
  int PlayerLowerHouse(Player player) const {
    // Player 0 starts at index 0; player 1 starts immediately after player 0.
  ...
  Action HouseToAction(int house) const {
    // Actions are row-relative. Both players therefore have actions 0..5,
    // even though their physical house indices are different.
    return house % num_houses_per_player_;
  }

  int ActionToHouse(Player player, Action action) const {
    // Convert a player's local action number into the global seeds[] index.
